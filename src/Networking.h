// the purpose of this header should be to provide SSL and networking wrapped in
// a common interface it should allow cross-platform networking and SSL and also
// easy usage of mTCP and similar tech
...
inline int setsockopt(SOCKET fd, int level, int optname, const void *optval,
                      socklen_t optlen) {
...
#include <unistd.h>
#define UWS_SOCKET_ERROR -1
#define UWS_INVALID_SOCKET -1
#define WIN32_EXPORT
#endif
...
// todo: probably merge this Context with the TLS::Context for same interface
// for SSL and non-SSL!
struct Context {
...
public:
  friend Context WIN32_EXPORT createContext(std::string certChainFileName,
  ...
  SSL_CTX *getNativeContext() { return context; }
};

Context WIN32_EXPORT createContext(std::string certChainFileName,
                                   std::string keyFileName,
                                   std::string keyFilePassword = std::string());

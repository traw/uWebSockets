...
enum ListenOptions : int { REUSE_PORT = 1, ONLY_IPV4 = 2 };
...
protected:
  ...
  template <void A(Socket *s)> static void accept_timer_cb(Timer *p) {
  ...
  static void accept_cb(ListenSocket *listenSocket) {
    ...
    if (clientFd == UWS_INVALID_SOCKET) {
       ...
       * polling will cause the server to spin, using 100% cpu. Switch to a
       * timer event instead to avoid this.
       */
      ...
      listenSocket->start(listenSocket->nodeData->loop, listenSocket,
                          UV_READABLE);
    }
    do {
      SSL *ssl = nullptr;
      ...
      Socket *socket = new Socket(listenSocket->nodeData,
                                  listenSocket->nodeData->loop, clientFd, ssl);
      socket->setPoll(UV_READABLE);
      A(socket);
    } while ((clientFd = netContext->acceptSocket(serverFd)) !=
             UWS_INVALID_SOCKET);
  }
...
public:
  ...
  Socket *connect(const char *hostname, int port, bool secure,
                  NodeData *nodeData) {
    ...
    uv_os_sock_t fd = netContext->createSocket(
  ...
  template <void A(Socket *s)>
  bool listen(const char *host, int port, uS::TLS::Context sslContext,
              int options, uS::NodeData *nodeData, void *user) {
    addrinfo hints, *result;
    memset(&hints, 0, sizeof(addrinfo));
    ...
    if ((options & uS::ONLY_IPV4) == 0) {
      for (addrinfo *a = result; a && listenFd == UWS_SOCKET_ERROR;
           a = a->ai_next) {
        if (a->ai_family == AF_INET6) {
          listenFd = netContext->createSocket(a->ai_family, a->ai_socktype,
                                              a->ai_protocol);
          listenAddr = a;
        }
      }
    ...
    if (::bind(listenFd, listenAddr->ai_addr, listenAddr->ai_addrlen) ||
        ::listen(listenFd, 512)) {
    ...
    ListenSocket *listenSocket =
        new ListenSocket(nodeData, loop, listenFd, nullptr);
